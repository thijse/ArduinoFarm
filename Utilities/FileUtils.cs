...
namespace Utilities
{
    public class FileUtils
    {
        ...
        public static bool MatchesFolder(string folderName, string folderWildcard)
        {
            folderName = Path.GetDirectoryName(folderName);
            if (string.IsNullOrEmpty(folderName)) return false;
            var wildcard = new Wildcard(folderWildcard, RegexOptions.IgnoreCase);
            return (wildcard.IsMatch(folderName));
        }


        public static bool MatchesFolder(string folderName, string[] folderWildcards)
        {
            return folderWildcards.Any(folder => MatchesFolder(folderName, folder));
        }

        public static bool MatchesFile(string fileName, string fileWildcard)
        {
            fileName = Path.GetFileName(fileName);
            if (string.IsNullOrEmpty(fileName)) return false;
            var wildcard = new Wildcard(fileWildcard, RegexOptions.IgnoreCase);
            return (wildcard.IsMatch(fileName));
        }

        public static bool MatchesFile(string fileName, string[] fileWildcards)
        {
            return (fileWildcards.Any(file => MatchesFile(fileName, file)));
        }
